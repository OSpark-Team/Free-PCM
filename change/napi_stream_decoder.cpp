...
napi_value PcmDecoderSetEqEnabled(napi_env env, napi_callback_info info) {
    ...
    // Apply EQ enabled immediately if EQ is already initialized
    if (ctx->eq.IsReady()) {
        ctx->eq.SetEnabled(enabled);
    }

    // Seek to current position to make EQ changes take effect immediately
    if (ctx->ring && ctx->eq.IsReady()) {
        uint64_t currentPosMs = ctx->ring->GetPositionMs();
        std::lock_guard<std::mutex> lock(ctx->seekMutex_);
        ctx->targetPositionMs_.store(static_cast<int64_t>(currentPosMs));
...
napi_value PcmDecoderSetEqGains(napi_env env, napi_callback_info info) {
    ...
    std::array<float, PcmEqualizer::kBandCount> gainsDb;
    for (uint32_t i = 0; i < len; i++) {
        ...
        gainsDb[i] = static_cast<float>(gainDb);
    ...
    // Apply EQ gains immediately if EQ is already initialized
    if (ctx->eq.IsReady()) {
        ctx->eq.SetGainsDb(gainsDb);
    }
...
napi_value PcmDecoderSetEqGainsLR(napi_env env, napi_callback_info info) {
    ...
    std::array<float, PcmEqualizer::kBandCount> gainsL;
    std::array<float, PcmEqualizer::kBandCount> gainsR;

    auto parseOne = [&](napi_value arr, size_t chIndex, std::array<float, PcmEqualizer::kBandCount>& gains) -> bool {
        bool isArray = false;
        ...
        for (uint32_t i = 0; i < len; i++) {
            ...
            gains[i] = static_cast<float>(gainDb);
            const int32_t gain100 = static_cast<int32_t>(std::lround(gainDb * 100.0));
    ...
    if (!parseOne(args[0], 0, gainsL) || !parseOne(args[1], 1, gainsR)) {
    ...
    // Apply EQ gains immediately if EQ is already initialized
    if (ctx->eq.IsReady()) {
        ctx->eq.SetGainsDbStereo(gainsL, gainsR);
    }
...
void ExecutePcmStreamDecode(napi_env /*env*/, void *data) {
    ...
    AudioDecoder::InfoCallback infoCb = [ctx](int32_t sr, int32_t cc, int32_t sf, int64_t durMs) {
        OH_LOG_INFO(LOG_APP, "infoCb received: sr=%{public}d, cc=%{public}d, sf=%{public}d, durMs=%{public}lld", sr, cc, sf, (long long)durMs);
        ctx->eqSampleRate = sr;
        ...
        // Apply saved EQ gains immediately after init
        {
        ...
        if (sf == 2) {
            ...
            OH_LOG_INFO(LOG_APP, "infoCb: S24LE (2) converted to S32LE (3)");
        } else {
            ctx->actualSampleFormat = sf;
            OH_LOG_INFO(LOG_APP, "infoCb: actualSampleFormat set to %{public}d", sf);
        }
        ...
        // S16LE=2 bytes, S24LE/S32LE/F32LE=4 bytes
        const int32_t bytesPerSample = (ctx->actualSampleFormat >= 3 || ctx->actualSampleFormat == 2) ? 4 : 2;
    ...
    AudioDecoder::PcmDataCallback pcmCb = [ctx](const uint8_t *pcm, size_t size, int64_t /*ptsMs*/) {
        ...
        // EQ and DRC support up to 8 channels, but per-channel volume only supports mono/stereo
        ...
        if (needChanVol) {
            ...
            // For multi-channel (>2), per-channel volume is not supported
        }

        // DRC and Limiter only support mono/stereo
        if (ch == 1 || ch == 2) {
            if (needDrc) {
        ...
        if (t == napi_object) {
            ...
            if (napi_get_named_property(env, args[1], "sampleFormat", &v) == napi_ok) {
                ...
                if (napi_get_value_int32(env, v, &sf) == napi_ok) {
                    // 0=auto, 1=S16LE, 2=S24LE, 3=S32LE, 4=F32LE
                    if (sf == 0 || sf == 1 || sf == 2 || sf == 3 || sf == 4) {
                        sampleFormat = sf;
                    } else {
                        sampleFormat = 0;
                    }
